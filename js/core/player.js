...
var Player = function(env, x, y) {
    ...
    this.captureDevice = function() {
        var capturedDevice = this.env.get(this.env.devices, this.getPosI(), this.getPosJ());
        
        if (capturedDevice) {
            this.hasDevice = true; // تفعيل الجهاز
            this.env.removeDevice(capturedDevice); // إزالة الجهاز من الخريطة
            return true;
        }
        return false;
    };
    ...
    this.update = function(keys) {
       ...
       this.markAsVisible();
        ...
        // التقاط الجهاز تلقائياً عند المشي عليه
        this.captureDevice(); 
